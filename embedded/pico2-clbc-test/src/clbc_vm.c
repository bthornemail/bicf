...
static void sha_update(bicf_sha256_ctx_t* ctx, const uint8_t* data, size_t len) {
  bicf_sha256_update(ctx, data, len);
}

static void emit_u8(bicf_sha256_ctx_t* ctx, uint8_t b) { sha_update(ctx, &b, 1); }

static void emit_uleb(bicf_sha256_ctx_t* ctx, uint64_t v) {
  uint8_t tmp[10];
  size_t n = uleb128_encode_u64(v, tmp);
  sha_update(ctx, tmp, n);
}
...
static bool consume_value_payload(const uint8_t* rs, size_t rs_len, size_t* idx, uint8_t vtype, bicf_sha256_ctx_t* sha) {
  ...
  if (vtype == 5) {
    ...
    sha_update(sha, rs + *idx, (size_t)n);
    *idx += (size_t)n;
    return true;
  }
  if (vtype == 6) {
    if (*idx + 4 > rs_len) return false;
    sha_update(sha, rs + *idx, 4);
    *idx += 4;
    return true;
  }
  return true;
  ...
  while (idx < rs_len) {
    ...
    switch (op) {
      ...
      case OP_PROJ_LINES: {
        ...
        emit_uleb(sha, n);
        sha_update(sha, rs + idx, (size_t)bytes);
        idx += (size_t)bytes;
        events++;
        break;
      }
