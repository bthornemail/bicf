...
static void sha256_compress(bicf_sha256_ctx_t* ctx, const uint8_t block[64]) {
  ...
  uint32_t w[64];
  for (uint32_t i = 0; i < 16; i++) w[i] = read_be32(block + (i * 4));
  for (uint32_t i = 16; i < 64; i++) w[i] = small_sigma1(w[i - 2]) + w[i - 7] + small_sigma0(w[i - 15]) + w[i - 16];
...
void bicf_sha256_final(bicf_sha256_ctx_t* ctx, uint8_t out32[32]) {
  ...
  ctx->bitlen += (uint64_t)ctx->block_len * 8;
  ...
  for (uint32_t i = 0; i < 8; i++) write_be32(out32 + (i * 4), ctx->state[i]);
}
