...
static void sha_update(mbedtls_sha256_context* ctx, const uint8_t* data, size_t len) {
  mbedtls_sha256_update(ctx, data, len);
}

static void emit_u8(mbedtls_sha256_context* ctx, uint8_t b) { sha_update(ctx, &b, 1); }
...
static bool consume_value_payload(const uint8_t* rs, size_t rs_len, size_t* idx, uint8_t vtype,
  ...
  // Unknown vtype: treat as empty payload (matches host behavior).
  return true;
}
...
static bool run_record_stream(const uint8_t* rs, size_t rs_len, clbc_vm_result_t* out, mbedtls_sha256_context* sha) {
  ...
  while (idx < rs_len) {
    ...
    switch (op) {
      ...
      default:
        // Unknown opcode: treat as failure (and do not emit it into transcript hash).
        out->events = events;
        return false;
    }
  }
