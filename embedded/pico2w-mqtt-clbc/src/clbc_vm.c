// Copy of embedded/pico2-clbc-test/src/clbc_vm.c (kept self-contained for Pico2W MQTT firmware).
#include "clbc_vm.h"
...
static bool read_bytes(const uint8_t* buf, size_t len, size_t* idx, uint8_t* out, size_t n) {
...
static bool skip_bytes(size_t len, size_t* idx, size_t n) {
  if (*idx + n > len) return false;
  *idx += n;
  return true;
}
...
static bool read_u32le(const uint8_t* buf, size_t len, size_t* idx, uint32_t* out) {
  uint8_t b[4];
  if (!read_bytes(buf, len, idx, b, 4)) return false;
...
static size_t uleb128_encode_u64(uint64_t v, uint8_t out10[10]) {
  size_t n = 0;
  do {
...
static void emit_u8(bicf_sha256_ctx_t* sha, uint8_t b) { bicf_sha256_update(sha, &b, 1); }

static void emit_uleb(bicf_sha256_ctx_t* sha, uint64_t v) {
  uint8_t tmp[10];
  size_t n = uleb128_encode_u64(v, tmp);
  bicf_sha256_update(sha, tmp, n);
}

static bool consume_value_payload(const uint8_t* rs, size_t rs_len, size_t* idx, uint8_t vtype, bicf_sha256_ctx_t* sha) {
  if (vtype == 0) return true;
  if (vtype == 1) {
    uint8_t b = 0;
    if (!read_u8(rs, rs_len, idx, &b)) return false;
    bicf_sha256_update(sha, &b, 1);
    return true;
  }
  if (vtype == 3 || vtype == 4) {
    uint64_t val = 0;
  ...
  if (vtype == 5) {
    ...
    bicf_sha256_update(sha, rs + *idx, (size_t)n);
    *idx += (size_t)n;
    return true;
  }
  if (vtype == 6) {
    if (*idx + 4 > rs_len) return false;
    bicf_sha256_update(sha, rs + *idx, 4);
    *idx += 4;
    return true;
  }
  return true;
...
static bool run_record_stream(const uint8_t* rs, size_t rs_len, clbc_vm_result_t* out, bicf_sha256_ctx_t* sha) {
  size_t idx = 0;
  uint32_t events = 0;
  ...
  while (idx < rs_len) {
    ...
    switch (op) {
      ...
      case OP_CTX_META: {
        ...
        uint8_t shape = 0;
        if (!read_u8(rs, rs_len, &idx, &shape)) return false;
      ...
      case OP_VTX_DEF: {
        ...
        uint8_t role = 0, basis = 0, optional = 0, vtype = 0;
        if (!read_u8(rs, rs_len, &idx, &role)) return false;
        if (!read_u8(rs, rs_len, &idx, &basis)) return false;
        if (!read_u8(rs, rs_len, &idx, &optional)) return false;
        if (!read_u8(rs, rs_len, &idx, &vtype)) return false;
      ...
      case OP_EDGE_DEF: {
        ...
        if (!uleb128_decode_u64(rs, rs_len, &idx, &sid)) return false;
        uint8_t wtype = 0;
        if (!read_u8(rs, rs_len, &idx, &wtype)) return false;
      ...
      case OP_KEY_DEF: {
        ...
        uint8_t auth = 0, mut = 0;
        if (!read_u8(rs, rs_len, &idx, &auth)) return false;
        if (!read_u8(rs, rs_len, &idx, &mut)) return false;
      ...
      case OP_PROJ_LINES: {
        ...
        emit_uleb(sha, n);
        bicf_sha256_update(sha, rs + idx, (size_t)bytes);
        idx += (size_t)bytes;
        events++;
        break;
      }
      ...
      case OP_COMMIT: {
        ...
        emit_uleb(sha, sid2);
        emit_uleb(sha, sid3);
        bicf_sha256_update(sha, rs + idx, 8);
        idx += 8;
        events++;
        break;
      }
...
bool clbc_vm_run(const uint8_t* clbc, size_t clbc_len, clbc_vm_result_t* out) {
  ...
  size_t idx = 4;
  uint8_t version = 0;
  if (!read_u8(clbc, clbc_len, &idx, &version)) return false;
  uint8_t flags = 0;
  ...
  if (!skip_bytes(clbc_len, &idx, st_len)) return false;
  const uint8_t* rs = clbc + idx;
  size_t rs_len = clbc_len - idx;

  bicf_sha256_ctx_t sha;
  bicf_sha256_init(&sha);
  bool ok = run_record_stream(rs, rs_len, out, &sha);
  bicf_sha256_final(&sha, out->transcript_sha256);
  out->ok = ok;
  return true;
}
